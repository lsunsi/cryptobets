...
contract Cryptobets {
  ...
  function createPool(uint256 _startTimestamp, uint256 _endTimestamp)
    public
    onlyOwner
    returns (uint256)
  {
  ...
  function placeBet(uint256 _poolId, BetOption _option) public payable {
    ...
    require(pools[_poolId].state == PoolState.Open, "Invalid pool state");
    require(
      _option == BetOption.Bull || _option == BetOption.Bear,
      "Invalid bet option"
    );

    // Should be extended to accept increasing bets
    require(
      bets[_poolId][msg.sender].option == BetOption.None,
      "Bet already placed"
    );

    bets[_poolId][msg.sender] = Bet({ option: _option, value: msg.value });
  ...
  function withdraw(uint256 _poolId) public payable {
    ...
    uint256 withdrawValue = ((pool.upBalance + pool.downBalance) *
      bet.value *
      99) / (winnerBalance * 100);
